"""MUMU-2 专用 Chess：沿用原动作，集中观察与单局状态。"""

import time

from module.exception import GameStuckError
from module.logger import logger
from tasks.Chess.script_task import ScriptTask as StableChessScriptTask
...
class ScriptTask(StableChessScriptTask):
    ...
    def _start_chess_game(self) -> None:
        ...
        logger.info(f'Chess test state: begin game {self.chess_state.game_number}')
    ...
    def _publish_chess_events(self, *, decision=None,
        ...
        for event in events:
            logger.debug(
                f'Chess test event: {event.kind.value}, '
...
for _legacy_name, _state_name in _GAME_STATE_FIELDS.items():
    setattr(ScriptTask, _legacy_name, _game_state_property(_state_name))
